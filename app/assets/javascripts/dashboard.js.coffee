...
ready = ->
  ...
  # -------------------------------
  # Decimal fields: allow typing partial numbers
  # -------------------------------
  $(document).on 'input', '.decimal-field', (e) ->
    # store raw input in dataset
    @dataset.rawValue = @value

  $(document).on 'blur', '.decimal-field', (e) ->
    raw = @dataset.rawValue
    val = parseFloat(raw)
    @value = if isNaN(val) then '' else val.toFixed(2)
